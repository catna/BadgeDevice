...
@property (nonatomic, assign) BOOL readEnable;
@end
...
- (void)distillData {
    ...
    if (self.FFCount <= FFCountMax && self.readEnable) {
        [self startDistill];
    } else {
        self.FFCount = 0;
        self.readEnable = NO;
...
- (void)parseCharacteristicData:(NSData *)data {
    ...
    *((char *)&uvle) = rawData[5];
    ...
    hisData.dataRecordTime = [TBluetoothTools parseHistoryDate:time];
